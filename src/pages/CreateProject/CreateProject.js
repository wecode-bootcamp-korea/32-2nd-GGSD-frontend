...
const CreateProject = () => {
  ...
  useEffect(() => {
    const getUrl = `http://10.58.3.182:8000/commons/meta`;
    fetch(getUrl)
      .then(res => res.json())
      .then(data => {
  ...
  const getThumbnailUrl = async () => {
    const api_url = `http://10.58.3.182:8000/commons/file`;
    ...
    try {
      ...
      console.log('이것은 이미지 못받은 에러다', err);
    }
  };

  const postProjectForm = async imgUrl => {
    const api_url = `http://10.58.3.182:8000/projects/enrollment`;
    ...
    try {
      const response = await fetch(api_url, {
        ...
        body: JSON.stringify({
          image_url: imgUrl || defaultThumbnail,
      ...
      <S.CreateForm>
        <S.Section textInput>
          <S.Name inputTitle isTitle>
            프로젝트 이미지 (썸네일 사이즈 : 650 x 400 , 가능 확장자 :
            .jpeg/.jpg/.png)
          </S.Name>
          <S.DefaultThumbLabel useClick>
            <S.DefaultThumb
              onChange={checkedDefaultThumbnail}
              type="checkBox"
            />
            기본이미지 사용
          </S.DefaultThumbLabel>
